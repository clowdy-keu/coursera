...
(для решения задачи разрешается использовать
числа с запятой, которые в Питоне пишутся через точку).
...
'''
kmPerDay, totalDistance = (int(input()) for _ in range(2))
finishDay = 1
while kmPerDay < totalDistance:
    kmPerDay *= 1.1
    finishDay += 1
print(finishDay)
